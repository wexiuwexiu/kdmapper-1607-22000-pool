...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, const std::wstring& driver_path, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader) {
	...
	intel_driver::remove_from_bigpool(iqvw64e_device_handle, kernel_image_base);
	...
	do {
		...
		if (free) {
			Log(L"[-] FreePool" << std::endl);
			intel_driver::SetMemory(iqvw64e_device_handle, realBase,0x0, nt_headers->OptionalHeader.SizeOfImage - TotalVirtualHeaderSize);
