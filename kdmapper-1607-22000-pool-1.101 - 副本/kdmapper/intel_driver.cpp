...
bool find_pool_table(HANDLE device_handle, uint64_t* pPoolBigPageTable, uint64_t* pPoolBigPageTableSize)//
{
	PVOID ExProtectPoolExCallInstructionsAddress = (PVOID)intel_driver::FindPatternInSectionAtKernel(device_handle, (char*)".text", intel_driver::ntoskrnlAddr, PUCHAR("\xE8\x00\x00\x00\x00\x83\x67\x0C\x00"), (char*)"x????xxxx");
	Log(L"[+] ExProtectPoolExCallInstructionsAddress 0x" << ExProtectPoolExCallInstructionsAddress << std::endl);

	if (!ExProtectPoolExCallInstructionsAddress)
		return false;

	PVOID ExProtectPoolExAddress = intel_driver::ResolveRelativeAddress(device_handle, ExProtectPoolExCallInstructionsAddress, 1, 5);

	if (!ExProtectPoolExAddress)
		return false;

	PVOID PoolBigPageTableInstructionAddress = (PVOID)((ULONG64)ExProtectPoolExAddress + 0x95);
	*pPoolBigPageTable = (uint64_t)intel_driver::ResolveRelativeAddress(device_handle, PoolBigPageTableInstructionAddress, 3, 7);

	PVOID PoolBigPageTableSizeInstructionAddress = (PVOID)((ULONG64)ExProtectPoolExAddress + 0x8E);
	*pPoolBigPageTableSize = (uint64_t)intel_driver::ResolveRelativeAddress(device_handle, PoolBigPageTableSizeInstructionAddress, 3, 7);
...
bool intel_driver::remove_from_bigpool(HANDLE device_handle, uint64_t Address)
{
	uint64_t pPoolBigPageTable = 0;
	uint64_t pPoolBigPageTableSize = 0;

	if (find_pool_table(device_handle, &pPoolBigPageTable, &pPoolBigPageTableSize))
	{
		PPOOL_TRACKER_BIG_PAGES PoolBigPageTable = 0;
		intel_driver::MemCopy(device_handle, (uintptr_t)&PoolBigPageTable, (uintptr_t)pPoolBigPageTable, 8);
		SIZE_T PoolBigPageTableSize = 0;
		intel_driver::MemCopy(device_handle, (uintptr_t)&PoolBigPageTableSize, (uintptr_t)pPoolBigPageTableSize, 8);

		Log(L"[+] PoolBigPageTableSize 0x" << PoolBigPageTableSize << std::endl);
		for (int i = 0; i < PoolBigPageTableSize; i++)
		{
			PVOID Va;
			intel_driver::MemCopy(device_handle, (uintptr_t)&Va, (uintptr_t)&PoolBigPageTable[i].Va, 8);
			//Log(L"[+] PoolBigPageTable[i].Va 0x" << Va << std::endl);
			ULONG Numbyte;
			intel_driver::MemCopy(device_handle, (uintptr_t)&Numbyte, (uintptr_t)&PoolBigPageTable[i].NumberOfBytes, 8);

			if (Va == (void*)Address || Va == (void*)(Address + 0x1))
			{
				Log(L"[+] PoolBigPageTable[i].Va 0x" << Va << std::endl);
				Log(L"[+] PoolBigPageTable[i].NumberOfBytes 0x" << Numbyte << std::endl);
				intel_driver::SetMemory(device_handle, (uintptr_t)&PoolBigPageTable[i].Va, (uintptr_t)0x1, 8);
				intel_driver::SetMemory(device_handle, (uintptr_t)&PoolBigPageTable[i].NumberOfBytes, (uintptr_t)0x0, 8);
				intel_driver::MemCopy(device_handle, (uintptr_t)&Va, (uintptr_t)&PoolBigPageTable[i].Va, 8);
				intel_driver::MemCopy(device_handle, (uintptr_t)&Numbyte, (uintptr_t)&PoolBigPageTable[i].NumberOfBytes, 8);
				Log(L"[+] PoolBigPageTable[i].Va 0x" << Va << std::endl);
				Log(L"[+] PoolBigPageTable[i].NumberOfBytes 0x" << Numbyte << std::endl);
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size) {
	...
	static uint64_t kernel_ExAllocatePool = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, "ExAllocatePool");
